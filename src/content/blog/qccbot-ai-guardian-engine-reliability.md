...
AI 异常恢复不是让系统一直乱点，直到页面变化。放在云手机工作流里，它应该表示：系统能识别一组有限的、已知的失败状态，执行安全范围内的处理，并把处理过程记录下来供团队复盘。

这个定义很重要。因为移动端自动化里，“加载慢”“出现权限弹窗”“账号进入恢复页面”不是同一种失败。如果全部混在一起，AI 就容易过度处理。

## 异常应该先分类，再决定能不能接管

团队最好先把异常分成几类，再决定 AI 可以做什么。

| 异常类型 | AI 适合做什么 | 人工边界 |
| --- | --- | --- |
| 加载慢 | 等待、有限重试、记录延迟 | 多次失败后停止 |
| 已知弹窗 | 按规则关闭或处理 | 新弹窗不应乱点 |
| UI 变化 | 保留上下文并提示脚本复查 | 不猜测新页面 |
| 账号登录 | 标记人工处理 | 不自动输入凭证 |
| 安全提醒 | 暂停并保留现场 | 必须人工判断 |

这种分类，比简单说“AI 自动恢复”更可信。真正可靠的自动化，不是 AI 更激进，而是规则更清楚、范围更有限、结果可复盘。

## 常见问题

### 云手机里的 AI Guardian Engine 是什么？

它可以理解为任务监控和异常分流层：观察脚本执行状态，识别常见失败，并把问题引导到重试、暂停、人工复核或脚本调整等不同处理方式。

### AI 应该恢复所有失败脚本吗？

不应该。AI 更适合处理已知、低风险、重复出现的问题。登录、支付、身份验证、账号安全提醒等情况仍然应该由人处理。

### 为什么异常分类这么重要？

没有分类时，所有问题都只是“脚本失败”。有了分类，团队能知道到底是加载慢、UI 变了、账号状态异常、弹窗干扰，还是需要人工复核。
